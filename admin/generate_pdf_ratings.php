...
class MYPDF extends TCPDF {
    ...
    public function ColoredTable($header, $data, $conn) {
        ...
        // Set column widths to match your ratings table structure
        $w = array(145, 50, 30, 45); // Adjust widths for other columns
        ...
        foreach ($data as $row) {
            ...
            // Fetch title based on news_id
            $title_query = "SELECT title FROM tbl_news WHERE id = ?";
            $stmt = mysqli_prepare($conn, $title_query);
            mysqli_stmt_bind_param($stmt, 'i', $row['news_id']);
            mysqli_stmt_execute($stmt);
            ...
            // Output the data
            $this->Cell($w[0], 6, $title, 'LR', 0, 'L', $fill); // Display title instead of news_id
            $this->Cell($w[1], 6, $username, 'LR', 0, 'L', $fill); // Display username instead of user_id
            $this->Cell($w[2], 6, $row['rating'], 'LR', 0, 'L', $fill);
...
// column titles
$header = array('GAME TITLE', 'NAME', 'RATINGS', 'CREATED_AT');
